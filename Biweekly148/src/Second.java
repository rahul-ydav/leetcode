...
public class Second {
    ...
    public class Graph{
        ...
        void dfs(int currNode, HashSet<Integer> set, int length){
//            if(visited[currNode]==1)    return;
            ...
            for(ArrayList<Integer> nodeData : edges.get(currNode)){
                if(visited[nodeData.get(0)]==0){
                    if(!set.contains(values[nodeData.get(0)])){
                        dfs(nodeData.get(0), set, length+nodeData.get(1));
                    }
                    else{
                        HashSet<Integer> set2 = new HashSet<>();
                        set2.add(values[currNode]);
                        dfs(nodeData.get(0), set2, nodeData.get(1));
                    }
                }
            }
            set.remove(values[currNode]);
            return;
        }
    }
